...
assert(max(checks{1,2:3})<1e-6 && checks.public_far_gap<1e-5,'Unvalidated exact reference.');
assert(contains(fileread(fullfile(reference_dir,'run.log')),'PARAMETER_EXPERIMENT_PASS project=contrast'), ...
    'The contrast study has not completed.');
